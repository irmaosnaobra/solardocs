...
- Corrente real de entrada de cada configuração (60/80/120/160 kW) — só a ficha técnica do fornecedor do carregador responde. Hoje a tabela do item 2.1 usa rendimento 0,95 arbitrado por mim. É o número que decide se um padrão de 150 A aguenta o 80 kW ou não, e é a promessa mais fácil de errar na frente do cliente.
- As quatro faixas de distância padrão → vaga (30 m / 80 m / 150 m). Nenhum arquivo da casa tem número de distância. A faixa decide se a vala entra no preço de tabela ou abre item separado no Anexo I — e ela também governa o rebaixamento de linha no placar de energia.
- Cláusula 7.4 ou 7.5 do contrato turnkey: parecer de acesso negado RETÉM o custo comprovado de projeto/ART/protocolo, ou vira CRÉDITO integral para outro local em até 12 meses? Para o público investidor (122 pessoas com capital e nenhum local) o crédito transforma um 'não' técnico em 'vamos achar outro ponto' — e é mais caro de honrar. Está aberta desde a v1 do contrato.
- O Estudo de Viabilidade passa a trazer a linha de DEMANDA CONTRATADA quando o ponto for Grupo A? O simulador hoje tem só assinat 0 + fixos R$300 + seguro 1% a.a. — não há demanda contratada em lugar nenhum. Se entrar, o payback de todo cenário em média tensão sobe, e a vantagem do 60 kW em baixa tensão fica maior do que a do item 2.2.
- Cravar uma taxa de conversão de fluxo contado → carros elétricos/dia, ou manter a conversão caso a caso dentro do estudo. É o que transforma a contagem da ficha em número de faturamento. Sem ela a ficha entrega o bruto e a reunião faz a conta. Não existe taxa em arquivo nenhum, e os números de mercado da LP (17,6%, 167.026) são de venda de veículo novo — não servem.
- Mínimo de vagas dedicadas: 2, ou a IO aceita fechar um ponto com 1 vaga exclusiva? Todas as quatro configurações têm 2 bicos e todo payback do simulador é calculado sobre os dois. Com 1 vaga, o teto de carros/dia cai pela metade.
- Pesos e cortes do PLACAR DO PONTO (24 = PONTO A, 16 = PONTO B, teto 30). Corte alto aperta a fila e manda mais gente para o cadastro de parceria; corte baixo enche a agenda de ponto que precisa de obra de entrada. Sugiro rodar ~15 vistorias reais e recalibrar com dado.
- O que fazer com a conclusão do item 2.2 — que abaixo de 20 carros/dia o 60 kW fatura igual ao 80 kW e paga um pouco mais rápido. Se ela virar regra da equipe, três artefatos ficam em desacordo com ela: o `CONFIGS[80].nota` da LP ('é a que indicamos na maioria dos pontos'), o atributo `selected` no 80 kW do simulador, e a apresentação comercial em Desktop/EletroPosto — que é a mesma tabela de preço e onde o deck manda no preço.

## Riscos conhecidos

- A conclusão do item 2.2 (60 kW ≈ 80 kW abaixo de 20 carros/dia) contradiz copy que está NO AR. O `CONFIGS[80].nota` da LP diz que o 80 kW 'é a que indicamos na maioria dos pontos', o `selected` do simulador abre nele, e a apresentação comercial usa a mesma tabela de preço. São três artefatos, não um. O capítulo está hedgeado (a ressalva de fila está escrita), mas se a equipe agir por ele sem alinhar os três, um consultor recomenda 60 kW numa reunião onde a tela mostra 80 kW recomendado.
- Três paybacks diferentes vão para a mesma mesa. O comentário do próprio simulador registra que o PDF do Estudo de Viabilidade ainda é feito sobre R$ 160.000 e que 'quem comparar os dois vê payback diferente' — a LP mostra 2,27 anos sobre R$ 144.595. As tabelas deste capítulo são um TERCEIRO conjunto derivado (2,3 / 2,2 / 2,7 anos). Conferi todas com a fórmula do arquivo, mas o consultor que levar o capítulo mais o PDF antigo tem números discordantes na frente do cliente. Refazer o estudo antes de circular.
- ATIVACAO diverge entre a fonte e a memória: o arquivo da LP tem R$ 1,20 por sessão e o índice de memória registra R$ 1,50. Usei 1,20 (o arquivo manda) e TODOS os paybacks deste capítulo dependem disso. Uma das duas está errada em algum outro lugar — provavelmente no deck ou no estudo.
- A régua dos 75 kW para média tensão é a de mercado, mas como cada distribuidora conta a carga instalada de um carregador DC está na NDU/NT dela. Marquei PONTO PARA O ADVOGADO. Se a contagem for diferente do que assumi, a seção 2.2 inteira muda de tamanho — e o erro caro é dizer ao cliente 'continua em baixa tensão' e o parecer voltar exigindo subestação (item 2.1.I, fora do escopo, por conta dele).
- Não sei o prazo regulatório da distribuidora para responder ao pedido de acesso. Está marcado como PONTO PARA O ADVOGADO no capítulo 3. Enquanto não houver resposta, ninguém da equipe pode prometer prazo de parecer — o contrato já protege a IO (Cl. 6.2.I e 7.1), mas a promessa verbal não.
- Não sei se estação de recarga aberta ao público tem exigência própria de vaga acessível, nem de quem é a obrigação (imóvel ou estação). Varia por município e tipo de edificação. Marcado como PONTO PARA O ADVOGADO no capítulo 6. Risco: exigência do município aparecer depois do comissionamento, com a obra entregue.
- A ficha e o placar nunca foram usados em campo. Os pesos são meus. Placar calibrado com opinião reprova ponto bom — por isso o fecho do documento pede ~15 vistorias reais antes de virar régua oficial.
- Se o placar virar campo de sistema, ele NÃO pode reaproveitar o campo `nota` da ficha do eletroposto. A NOTA 1/2/3 em produção mede o LEAD e alimenta ficha, alerta de WhatsApp, pixel e a função ep_motivos() no banco. PONTO A/B/C/X mede o LOCAL. Misturar os dois é exatamente o desalinhamento que o comentário do código já avisa que quebra a página de destino.