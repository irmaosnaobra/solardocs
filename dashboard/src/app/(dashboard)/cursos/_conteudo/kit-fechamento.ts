...
import { XP_POR_TIPO as XP, type Curso, type ModuloCurso, type TipoLicao } from './curso-tipos';
import { OBJECOES, TOTAL_OBJECOES } from './objecoes';
import { PROSPECCAO } from './prospeccao';
import { POS_VENDA } from './posvenda';

// Módulos normais. O bônus entra depois, porque o requisito dele precisa saber
// quantas lições existem aqui — e esse número tem que sair da conta, não da
// memória de quem editou por último.
const MODULOS: ModuloCurso[] = [
  {
    slug: 'objecoes',
    numero: 1,
    titulo: 'As objeções que travam a venda',
    subtitulo: `A resposta pronta para as ${TOTAL_OBJECOES} frases que aparecem depois da proposta`,
...
/** Lições dos módulos normais — o que o bônus exige para destravar. */
export const TOTAL_LICOES = MODULOS.reduce((n, m) => n + m.licoes.length, 0);

// ── BÔNUS ────────────────────────────────────────────────────────────────────
// Travado até o aluno terminar o curso E estar usando a plataforma de verdade.
// É a recompensa que puxa para dentro do produto: o cara só destrava depois de
// cadastrar empresa, cliente e gerar um documento.
const BONUS_PJ: ModuloCurso = {
  slug: 'bonus-pj',
  ...
  requisitos: [
    {
      id: 'licoes',
      label: `Concluir as ${TOTAL_LICOES} lições do curso`,
      dica: `Os ${MODULOS.length} módulos, do primeiro ao último`,
    },
    {
      id: 'empresa',
