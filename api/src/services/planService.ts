...
export async function checkLimit(userId: string): Promise<void> {
  ...
  if (user.documentos_usados >= user.limite_documentos) {
    throw new ApiError(403, `Limite de documentos atingido. Seu plano ${user.plano.toUpperCase()} permite ${user.limite_documentos} documento(s) por mês. Faça upgrade para o plano PRO ou VIP para continuar.`);
  }
}
...
export async function runMonthlyReset(): Promise<void> {
  ...
  const { data, error } = await supabase
    .from('users')
    .update({ documentos_usados: 0, data_reset: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() })
    .eq('plano', 'pro')
    .lte('data_reset', now)
