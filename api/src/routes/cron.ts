...
// ── Régua do SIM (eletroposto) ───────────────────────────────────────────────
// Quem marcou e não confirmou é cobrado em +1h e +2h, e na terceira o horário
// volta pra vitrine e a ficha vai pro Curioso. ?dry=1 mostra quem seria cobrado
// e quem perderia o horário, sem enviar, sem cancelar e sem criar ficha.
router.get('/eletroposto-cobra-sim', async (req: Request, res: Response) => {
  if (!verifyCronSecret(req, res)) return;
  try {
    const dry = req.query.dry === '1' || req.query.dry === 'true';
    res.json({ ok: true, dry, ...(await runEletropostoCobraSimTick({ dry })) });
  } catch (err: any) {
    logger.error('cron', 'eletroposto-cobra-sim falhou', err);
    res.status(500).json({ error: 'Cron failed', detail: String(err?.message || err) });
  }
});

// ── Follow-up de retorno (eletroposto) ───────────────────────────────────────
...
router.get('/eletroposto-retorno', async (req: Request, res: Response) => {
  ...
  try {
    const dry = req.query.dry === '1' || req.query.dry === 'true';
    res.json({ ok: true, dry, ...(await runEletropostoRetornoTick({ dry })) });
  } catch (err: any) {
    logger.error('cron', 'eletroposto-retorno falhou', err);
    res.status(500).json({ error: 'Cron failed', detail: String(err?.message || err) });
  }
});
...
// 9h–19h, até 2 reagendamentos por ficha, e SÓ pra lead quente (nota 3).
router.get('/eletroposto-reagenda-auto', async (req: Request, res: Response) => {
  if (!verifyCronSecret(req, res)) return;
  try {
    const dry = req.query.dry === '1' || req.query.dry === 'true';
    res.json({ ok: true, dry, ...(await runEletropostoReagendaAutoTick({ dry })) });
  } catch (err: any) {
    logger.error('cron', 'eletroposto-reagenda-auto falhou', err);
    res.status(500).json({ error: 'Cron failed', detail: String(err?.message || err) });
  }
});
