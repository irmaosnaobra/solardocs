...
// Para a base que JÁ está dentro (free e PRO). O gancho não é desconto: é que o
// curso — o mesmo que é vendido por R$27 lá fora — já vem incluído no plano.
//
// Cadência própria (campanha_curso_count), separada do upgrade_nudge: aquele tem
// teto de 3 toques que boa parte da base já consumiu, e zerar o contador dele
// contaminaria a métrica do nudge que roda o ano inteiro.
//
// A oferta manda pro checkout `vip_curso`: mesmo preço do VIP, cobrança imediata
// (sem os 7 dias) — a contrapartida honesta de "o curso entra junto".
const CAMPANHA_CURSO = [
  {
    ...
    cta: 'Assinar o VIP e abrir o curso →',
  },
  {
    subject: 'A objeção que mais derruba orçamento solar (e a resposta pronta)',
    titulo: '"Achei mais barato com outro"',
    corpo: `É a objeção que mais derruba proposta de energia solar — e quase sempre a resposta é
            baixar o preço, que é como se perde a margem do ano inteiro.
            <br/><br/>
            A lição 1 do Kit de Fechamento é inteira sobre ela: o que responder, em que ordem, e a
            versão curta pra mandar no WhatsApp. O curso é vendido por R$ 27 e
            <strong>entra junto no VIP</strong>, com os documentos ilimitados.`,
    cta: 'Ver o VIP com o curso incluso →',
  },
  {
    subject: 'Última chamada: curso + documentos ilimitados por R$67/mês',
    titulo: 'Fecha mais, e a papelada sai sozinha',
    corpo: `Duas coisas travam venda solar: não saber responder a objeção na hora, e demorar dois dias
            pra mandar o contrato.
            <br/><br/>
            O VIP resolve as duas: <strong>curso de fechamento incluso</strong> e documentos
            ilimitados com o CNPJ e a logo da sua empresa. R$ 67/mês, cancela quando quiser.`,
    cta: 'Assinar o VIP →',
  },
];

export async function sendCampanhaCursoVipEmail(
  email: string,
  userId: string,
  toque: number,
  nome?: string | null,
  ...
  const html = `
<div style="font-family:'Segoe UI',Arial,sans-serif;max-width:600px;margin:0 auto;background:#0f172a;border-radius:16px;overflow:hidden;">
  <div style="background:linear-gradient(135deg,#f59e0b 0%,#fbbf24 100%);padding:30px 34px;">
    <p style="margin:0;color:#0f172a;font-size:12px;font-weight:800;letter-spacing:2px;text-transform:uppercase;">SolarDoc VIP</p>
  ...
  <div style="padding:30px 34px;">
    ...
    <p style="color:#64748b;font-size:12.5px;line-height:1.6;margin:16px 0 0;text-align:center;">
      A cobrança começa na assinatura (sem os 7 dias de teste) — é o que paga o curso ir junto.
    </p>
  </div>
</div>`.trim();

  await sendMarketingEmail({ to: email, userId, subject: t.subject, html });
}
